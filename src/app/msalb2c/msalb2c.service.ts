...
import {
    ...
    PopupRequest,
    RedirectRequest,
    SilentRequest, AuthError
} from "@azure/msal-browser";
import { InteractionType, MSALB2C_INSTANCE } from "./constants";
...
export class MsalB2CService {
    ...
    constructor(
        @Inject(MSALB2C_INSTANCE) private msalB2CInstance: IPublicClientApplication,
    ...
    public acquireTokenPopup(request: AuthorizationUrlRequest): Observable<AuthenticationResult> {
        return from(this.msalB2CInstance.acquireTokenPopup(request));
    }
    
    public acquireTokenRedirect(request: RedirectRequest): Observable<void> {
        return from(this.msalB2CInstance.acquireTokenRedirect(request));
    }
    
    public acquireTokenSilent(silentRequest: SilentRequest): Observable<AuthenticationResult> {
        return from(this.msalB2CInstance.acquireTokenSilent(silentRequest));
    }
